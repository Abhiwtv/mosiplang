    ...
    return (
      <div className="max-w-6xl mx-auto p-6">
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-16 text-center shadow-sm">
          ...
          <h3 className="text-lg font-semibold text-gray-900 mb-2">{t('noPassportsTitle')}</h3>
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="bg-emerald-50 px-4 py-2 rounded-full border border-emerald-100">
          <span className="text-emerald-800 font-medium">
            {certificates.length} {t('active')} {certificates.length === 1 ? t('passportSingular') : t('passportPlural')}
          </span>
        </div>
      </div>
              ...
              <div className="bg-gradient-to-br from-emerald-600 to-emerald-800 p-6 text-white relative overflow-hidden">
                ...
                <div className="flex items-start justify-between mb-4 relative z-10">
                  ...
                  <span className={`px-3 py-1 text-xs font-medium rounded-full border backdrop-blur-sm ${
                    ...
                    {isExpired ? t('expired') : t('verified')}
                  </span>
                </div>
              ...
              <div className="p-6 space-y-4">
                <div className="space-y-3">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="text-xs text-gray-500 uppercase tracking-wide">{t('exporter')}</p>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="text-xs text-gray-500 uppercase tracking-wide">{t('issuedDate')}</p>
            ...
            <div className="p-8 space-y-8">
              ...
              <div className="flex flex-col sm:flex-row gap-6 items-start">
                ...
                <div className="flex-1 space-y-4 w-full">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <p className="text-xs text-gray-500 uppercase font-bold tracking-wider">{t('modal.batchId')}</p>
                      <p className="text-base font-bold text-gray-900">{selectedCert.batch_number}</p>
                    </div>
                    <div>
                      <p className="text-xs text-gray-500 uppercase font-bold tracking-wider">{t('modal.product')}</p>
              ...
              {(() => {
                ...
                return (
                  <div className="bg-gray-50 rounded-xl p-6 border border-gray-200">
                    <h3 className="text-sm font-bold text-gray-800 uppercase mb-4 tracking-wide">{t('modal.qualityTitle')}</h3>
                    <div className="grid grid-cols-2 gap-x-8 gap-y-4">
                      ...
                      <div className="flex justify-between items-center border-b border-gray-200 pb-2">
                        <span className="text-gray-600 text-sm">{t('modal.qualityGrade')}</span>
                        <span className="font-bold text-emerald-600 bg-emerald-50 px-2 rounded">{t('grade')} {q.grade || 'A'}</span>
                      </div>
                      <div className="flex justify-between items-center border-b border-gray-200 pb-2">
                        ...
                        <span className={`font-semibold ${q.organic ? 'text-emerald-600' : 'text-gray-900'}`}>
                          {q.organic ? t('modal.yes') : t('modal.no')}
                        </span>
                      </div>
                    </div>
                  </div>
              ...
              {selectedCert.batch_details && (
                <div className="bg-blue-50/50 rounded-xl p-6 border border-blue-100">
                  <h3 className="text-sm font-bold text-blue-900 uppercase mb-4 tracking-wide">{t('modal.logisticsTitle')}</h3>
              ...
              <div className="text-center pt-4 border-t border-gray-100">
                <p className="text-xs text-gray-400">
                  {t('modal.signedBy')} <span className="font-medium text-gray-600">{selectedCert.credential_data?.issuer?.name || 'QA Authority'}</span>
                </p>
